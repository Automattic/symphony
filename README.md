...
For issue-mode runs, Symphony bootstraps Linear before the first agent turn by moving `Todo` issues
to `In Progress` and creating the configured workpad comment if one is not already present. Agents
still own reconciling and updating that workpad during execution.
