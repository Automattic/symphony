...
- When `verification.dev_server.start_cmd` is set, Symphony starts it in the issue workspace after
  ...
  after `stop_timeout_ms`. A hook-started dev server still works, but it is outside Symphony's
  supervision and health gate; such hook scripts must manage their own backgrounding and cleanup.
