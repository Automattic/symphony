defmodule SymphonyElixir.ClaudeCode.AppServer do
  ...
  @doc false
  @spec build_sandbox_settings(Agent.NetworkAccess.t(), [String.t()], [String.t()]) :: map()
  def build_sandbox_settings(%Agent.NetworkAccess{mode: mode} = network_access, allow_read_paths \\ [], allow_write_paths \\ []) do
    base =
      %{
        "sandbox" => %{
          "enabled" => true,
          ...
          "filesystem" => AgentSandboxConfig.claude_filesystem_settings(allow_read_paths, allow_write_paths)
        }
      }
  ...
  defp build_claude_settings(network_access, allow_read_paths, allow_write_paths) do
    network_access
    |> build_sandbox_settings(allow_read_paths, allow_write_paths)
    |> Map.put("permissions", %{
      "deny" => [
        "Bash(gh:*)",
        "Bash(git push:*)",
  ...
  defp write_claude_runtime_files(_workspace, worker_host, settings, mcp_session, socket_path, remote_shim_path) do
    ...
    settings_json = build_claude_settings(network_access, allow_read_paths, allow_write_paths)
